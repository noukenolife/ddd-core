...
import noukenolife.ddd.domain.api.lifecycle.{EntityNotFoundException, IOContext, Repository}
import noukenolife.ddd.domain.api.model.{FakeEntity, FakeId}

class FakeRepository(var entityMap: Map[FakeId, FakeEntity] = Map.empty) extends Repository[FakeId, FakeEntity, IO] {
